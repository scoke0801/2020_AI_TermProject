...
//----------------------------- Graph_SearchRDFS -------------------------------
//
//  class to implement a Iterativ Deepening depth first search. 
//-----------------------------------------------------------------------------
template<class graph_type>
class Graph_SearchRDFS
{
private:
...
public:
    ...
    Graph_SearchRDFS(const graph_type& graph,
        int          source,
        int          target = -1) :
...
//-----------------------------------------------------------------------------
template <class graph_type>
bool Graph_SearchRDFS<graph_type>::Search()
{/*
...
//--------------------------------------------------------------------------
    ...
    vector<int> lengths;
    lengths.reserve(19 * 19);
    for (int i = 0; i < 19 * 19; ++i) lengths.push_back(0);
    while (true)
    {
        //create a dummy edge and put on the stack
        Edge Dummy(m_iSource, m_iSource, 0);
        ...
        while (!stack.empty())
        {
            ...
            if (lengths[Next->To()] > lenLimit) continue;
...
//-----------------------------------------------------------------------------
template <class graph_type>
std::list<int> Graph_SearchRDFS<graph_type>::GetPathToTarget()const
{
    std::list<int> path;
...
public:
  ...
  Graph_SearchDijkstra(const graph_type   &graph,
                       ...
                       int           target = -1):m_Graph(graph),
                                       ...
                                       m_iSource(source),
                                       m_iTarget(target)    // 찾을대상, target이 없으면 모든 노드에 대해서 탐색
  {                                           
    Search();     
  }
...
void Graph_SearchDijkstra<graph_type>::Search()
{
    ...
    while (!pq.empty())
    {
            ...
            pE = ConstEdgeItr.next())
        {
            ...
            // 이미 기록이 되어있는 경우에 새로운 경로가 발견이 된 경우.

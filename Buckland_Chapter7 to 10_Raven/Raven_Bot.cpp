...
void Raven_Bot::Update()
{
    ...
    // steering class를 통해 이동을 처리.
    UpdateMovement();

    //if the bot is under AI control but not scripted
    // 사용자가 컨트롤 하는 물체가 아니라면
    if (!isPossessed())
    {
        //examine all the opponents in the bots sensory memory and select one
        //to be the current target
        // Regulator: 한번에 싸이클이 다 돌면 버벅일수 있으므로
        // 순서를 조절해주는 기능.
        ...
        // 평가를 통해 우선순위 할당. 
        // 매 frame마다 Goal이 바뀌면 안되므로 Regulator에서 이를 조절해줌.
        ...
        // to use from the weapons currently in the inventory
        // 무기선택(퍼지)
